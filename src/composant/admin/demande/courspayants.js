...
const CoursPayantListAdminD = () => {
  ...
  useEffect(() => {
    const fetchCours = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/coursPayant/getAllcourssDemandeAdmin`);
  ...
  const handleSearchChange = async (event) => {
    ...
    try {
      const response = await axios.get(`http://localhost:3000/coursPayant/searchcourssByTitreAdmin?titre=${searchTerm}`);
      setCours(response.data.courss || []); // Note: Changed to response.data.courss
    } catch (error) {
      console.error('Erreur lors de la récupération des cours :', error);
    }
  };
  ...
  const handleAccept = async (id) => {
    try {
      await axios.put(`http://localhost:3000/coursPayant/accepter/${id}`);
      setCours(prevCours => prevCours.map(c => c.id_cp === id ? { ...c, accepted: true } : c));
  ...
  const handleReject = async (id) => {
    try {
      await axios.put(`http://localhost:3000/coursPayant/refuser/${id}`);
      setCours(prevCours => prevCours.map(c => c.id_cp === id ? { ...c, rejected: true } : c));
  ...
  const filteredCours = cours.filter(c =>
    !getAcceptedCours().includes(c.id_cp) && !getRejectedCours().includes(c.id_cp)
  );
  ...
  const highlightText = (text, highlight) => {
    ...
    return (
      <span>
        {parts.map((part, i) => 
          part.toLowerCase() === highlight.toLowerCase() ? <span key={i} style={{ color: 'red', textDecoration: 'underline' }}>{part}</span> : part
        )}
      </span>
    );
  };
  ...
  return (
    <div className="cours-list-admin" style={{ backgroundColor: '#eee', marginTop: '20px' }}>
      ...
      <h1 style={{ color: '#5e4803', fontFamily: 'Roboto, sans-serif', fontSize: '40px', marginTop: '0px' }}>
        Les demandes de création des Cours
      </h1>
        ...
        <MDBTableBody>
          {currentItems.map((cours) => (
            <tr key={cours.id_cp}>
              ...
              <td>{highlightText(cours.titre, searchQuery)}</td>
              <td>{cours.description}</td>
              <td>
                <MDBBtn color='success' rounded size='sm' padding='10px' onClick={() => handleAccept(cours.id_cp)}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check-circle-fill" viewBox="0 0 16 16">
                    <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0m-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/>
                  </svg>
                </MDBBtn>
                <MDBBtn color='danger' rounded size='sm' padding='10px' onClick={() => handleReject(cours.id_cp)}>
...
export default CoursPayantListAdminD;
