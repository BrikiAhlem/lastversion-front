...
const FormationAdminD = () => {
  ...
  const handleSearchChange = async (value) => {
    ...
    try {
      const response = await axios.get(`http://localhost:3000/formationP/searchByDomaineAdmin?domaine=${value}`);
  ...
  const highlight = (text, highlight) => {
    ...
    return parts.map((part, index) =>
      part.toLowerCase() === highlight.toLowerCase() ? <span key={index} style={{ backgroundColor: 'red' }}>{part}</span> : part
    );
  };
  ...
  return (
    <div className="container d-flex justify-content-center">
      <div className="Formation-list-admin" style={{ backgroundColor: '#eee', marginTop: '20px' }}>
        ...
        <h1 className="text-center" style={{ color: '#5e4803', fontFamily: 'Roboto, sans-serif', fontSize: '40px', marginTop: '0px' }}> les demandes de création des formations</h1>
            ...
            <MDBTableBody>
              {currentItems.map((formation) => (
                <tr key={formation.id_fp}>
                  ...
                  <td>{highlight(formation.domaine, searchQuery)}</td>
