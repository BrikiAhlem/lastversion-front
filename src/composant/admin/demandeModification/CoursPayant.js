...
const CoursPayantAdminModifier = () => {
  ...
  useEffect(() => {
    const fetchCours = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/coursgratuis/getAllcourssAdminModifier`);
  ...
  const handleSearchChange = async (event) => {
    ...
    try {
      const response = await axios.get(`http://localhost:3000/coursPayant/searchcourssByTitreAdminmodification?titre=${searchTerm}`);
      setCours(response.data.courss || []);
    } catch (error) {
      console.error('Erreur lors de la récupération des cours :', error);
    }
  };
...
export default CoursPayantAdminModifier;
