  ...
  const handleSubmitRecherche = async (e) => {
    ...
    try {
      if (TitreRecherche.trim() !== '') {
        const response = await axios.get(`http://localhost:3000/coursgratuis/rechercherByTitre/${instructeurId}?titre=${TitreRecherche}`);
  ...
  const searchButtonStyle = {
    ...
    <div style={{ backgroundColor: '#efefef', padding: "20px", paddingBottom: "90px" }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', justifyContent: 'space-between', margin: '30px', marginTop: "80px" }}>
        ...
        {ShowModalS && selectedcours && (
  <MDBModal open={ShowModalS} onClose={handleCloseModal}>
    <MDBModalDialog>
      <MDBModalContent style={{ backgroundColor: '#fff', borderRadius: '8px', boxShadow: '0 8px 16px rgba(0, 0, 0, 0.1)', padding: '20px' }}>
        <MDBModalHeader style={{ backgroundColor: '#007bff', color: '#fff', borderRadius: '8px 8px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <Input
        ...
        required
        style={{ border: '1px solid #ced4da', borderRadius: '4px', padding: '8px', width: '100%', marginBottom: '10px' }}
      />
    </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '20px' }}>
              <button type="button" style={{ backgroundColor: '#dc3545', color: '#fff', border: 'none', borderRadius: '4px', padding: '8px 16px', cursor: 'pointer', marginRight: '10px', transition: 'background-color 0.3s ease' }} onClick={handleCloseModal}>Annuler</button>
